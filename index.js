...
const ExternalLink = ({ children, icon, color, ...props }) => (
    <Link target="_blank" rel="noopener noreferrer" color={color} {...props}>
        {icon && <Icon icon={icon} sibling={children} size="lg" fixedWidth />}
        {children}
    </Link>
);
    ...
    const footerItems = [
        {
            children: (
                <>
                    ...
                    <IMRDLogo xmlns="http://www.w3.org/2000/svg">
        ...
        [
            {
                title: 'Twitter',
                get href() {
                    return `${twitterUrl}/${this.tweet.handle}`;
                },
                icon: faTwitter,
                color: twitterColor
            },
